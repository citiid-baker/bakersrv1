...
#ncbi-genome-download -s refseq -F fasta,gff -l complete,chromosome -o ${LOCATION} -H -P -p $THREADS -r 3 -m ${LOCATION}/host_references.tsv -g "Homo sapiens,Mus musculus" -N vertebrate_mammalian
...
grep \$TERM ${LOCATION}/references.tsv | awk -F"\t" 'BEGIN {print "\"Species\",\"Reference File\""} {print \$10" "\$11","\$23}'
END
